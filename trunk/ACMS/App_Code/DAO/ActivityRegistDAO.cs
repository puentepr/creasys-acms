...
namespace ACMS.DAO
{
    public class ActivityRegistDAO : BaseDAO
    {
        ...
        public void InsertActivityRegistCancel(Guid activity_id, string emp_id, string activity_type, string cancel_by,SqlConnection conn)
        {
...
//sqlParams[8].Value = myActivityRegistVO.idno_ext;
        ...
        public int DeleteRegist(Guid activity_id, string emp_id, string activity_type, string webPath ,string allTeams,string RegWebPath)
        {
            ...
            using (SqlConnection myConn = MyConn())
            {
                ...
                try
                {
                    ...
                    if (activity_type == "2")
                    {
                        sb.AppendLine("select boss_id from  ActivityTeamMember  WHERE activity_id=@activity_id and emp_id in (SELECT * FROM dbo.UTILfn_Split(@emp_id,',')); ");
                        //myConn.Open();
                        cmd.Connection = myConn;
                        cmd.CommandText = sb.ToString();
                        cmd.Parameters.Clear();
                        ...
                        {
                            bossid = ds.Tables[0].Rows[0]["boss_id"].ToString();
                    ...
                    else
                    {
                        ...
                        sb.AppendLine("DELETE A FROM CustomFieldValue A inner join CustomField B on A.field_id=B.field_id WHERE B.activity_id=@activity_id and A.emp_id in (SELECT * FROM dbo.UTILfn_Split(@emp_id,',')); ");
                        //要檢查是否全數已殺光 andy
                        sb.AppendLine("  declare @cc int ");
                        sb.AppendLine(" select @cc=COUNT(*) from ActivityTeamMember  where activity_id =@activity_id and boss_id ='" + bossid + "'");
                        sb.AppendLine(" if @cc =0 delete from  ActivityRegist where activity_id =@activity_id and emp_id ='" + bossid + "'");
                ...
                using (SqlConnection myConn = MyConn())
                {
                    ...
                    try
                    {
                        ...
                        if (activity_type == "2")
                        {
                            ...
                            if (cmd.ExecuteScalar().ToString() != "0")
                            {
                                InsertActivityRegistCancel(activity_id, OriginMembers, "2", clsAuth.ID, myConn);
                            }
                            //================================================
        ...
        public string AllTeamMemberByMembers(Guid activity_id, string emp_id,SqlConnection conn)
        {
            SqlParameter[] sqlParams = new SqlParameter[2];
            ...
            SqlDataReader MyDataReader = SqlHelper.ExecuteReader(conn, CommandType.Text, sb.ToString(), sqlParams);
